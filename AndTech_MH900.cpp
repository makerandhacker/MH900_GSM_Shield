...
#if ARDUINO >= 100
AndTech_MH900::AndTech_MH900(SoftwareSerial *ss, int8_t rst)
#else
AndTech_MH900::AndTech_MH900(NewSoftSerial *ssm, int8_t rst) 
#endif
{
  _rstpin = rst;
  mySerial = ss;
...
boolean AndTech_MH900::begin(uint16_t baudrate) {
...
boolean AndTech_MH900::getBattVoltage(uint16_t *v) {
  return sendParseReply(F("AT+CBC"), F("+CBC: "), v, ',', 2);
}

boolean AndTech_MH900::getADCVoltage(uint16_t *v) {
  return sendParseReply(F("AT+CADC?"), F("+CADC: 1,"), v);
}

/********* SIM ***********************************************************/

uint8_t AndTech_MH900::unlockSIM(char *pin)
{
...
uint8_t AndTech_MH900::getSIMCCID(char *ccid) {
   getReply("AT+CCID");
   // up to 20 chars
...
uint8_t AndTech_MH900::getNetworkStatus(void) {
  uint16_t status;
...
uint8_t AndTech_MH900::getRSSI(void) {
  uint16_t reply;
...
boolean AndTech_MH900::setAudio(uint8_t a) {
...
uint8_t AndTech_MH900::getVolume(void) {
  uint16_t reply;
...
boolean AndTech_MH900::setVolume(uint8_t i) {
  return sendCheckReply(F("AT+CLVL="), i, F("OK"));
}


boolean AndTech_MH900::playToolkitTone(uint8_t t, uint16_t len) {
  return sendCheckReply(F("AT+STTONE=1,"), t, len, F("OK"));
}

boolean AndTech_MH900::setMicVolume(uint8_t a, uint8_t level) {
  // 0 is headset, 1 is external audio
  if (a > 1) return false;
...
boolean AndTech_MH900::FMradio(boolean onoff, uint8_t a) {
  if (! onoff) {
...
boolean AndTech_MH900::tuneFMradio(uint16_t station) {
...
boolean AndTech_MH900::setFMVolume(uint8_t i) {
...
int8_t AndTech_MH900::getFMVolume() {
  uint16_t level;
...
int8_t AndTech_MH900::getFMSignalLevel(uint16_t station) {
  // Fail if FM station is outside allowed range.
  ...
  getReply(F("AT+FMSIGNAL="), station, MH900_DEFAULT_TIMEOUT_MS);
...
boolean AndTech_MH900::PWM(uint16_t period, uint8_t duty) {
...
/********* CALL PHONES **************************************************/
boolean AndTech_MH900::callPhone(char *number) {
...
boolean AndTech_MH900::hangUp(void) {
  return sendCheckReply(F("ATH0"), F("OK"));
}

/********* SMS **********************************************************/

int8_t AndTech_MH900::getNumSMS(void) {
  uint16_t numsms;
...
// printouts!
boolean AndTech_MH900::readSMS(uint8_t i, char *smsbuff, 
...
boolean AndTech_MH900::sendSMS(char *smsaddr, char *smsmsg) {
...
boolean AndTech_MH900::deleteSMS(uint8_t i) {
    if (! sendCheckReply("AT+CMGF=1", "OK")) return -1;
  // read an sms
...
boolean AndTech_MH900::enableGPRS(boolean onoff) {
  ...
  if (onoff) {
    ...
    if (! sendCheckReply(F("AT+SAPBR=3,1,\"APN\",\"MH900net\""),
			   F("OK"), 10000))
      return false;
...
uint8_t AndTech_MH900::GPRSstate(void) {
  uint16_t state;
...
boolean AndTech_MH900::getGSMLoc(uint16_t *errorcode, char *buff, uint16_t maxlen) {
...
boolean AndTech_MH900::HTTP_GET_start(char *url, 
  ...
  mySerial->print(url);
  mySerial->println("\"");
  readline(MH900_DEFAULT_TIMEOUT_MS); 
  if (strcmp(replybuffer, "OK") != 0)
    return false;
...
boolean AndTech_MH900::HTTP_GET_end(void) {
  flushInput();
...
inline int AndTech_MH900::available(void) {
  return mySerial->available();
}

inline size_t AndTech_MH900::write(uint8_t x) {
  return mySerial->write(x);
}

inline int AndTech_MH900::read(void) {
  return mySerial->read();
}

inline int AndTech_MH900::peek(void) {
  return mySerial->peek();
}

inline void AndTech_MH900::flush() {
  mySerial->flush();
}

void AndTech_MH900::flushInput() {
...
uint16_t AndTech_MH900::readRaw(uint16_t b) {
  uint16_t idx = 0;
...
uint8_t AndTech_MH900::readline(uint16_t timeout, boolean multiline) {
  uint16_t replyidx = 0;
...
uint8_t AndTech_MH900::getReply(char *send, uint16_t timeout) {
  flushInput();
...
uint8_t AndTech_MH900::getReply(const __FlashStringHelper *send, uint16_t timeout) {
  flushInput();
...
uint8_t AndTech_MH900::getReply(const __FlashStringHelper *prefix, char *suffix, uint16_t timeout) {
  flushInput();
...
// Send prefix, suffix, and newline. Return response (and also set replybuffer with response).
uint8_t AndTech_MH900::getReply(const __FlashStringHelper *prefix, int32_t suffix, uint16_t timeout) {
  flushInput();
...
// Send prefix, suffix, and newline. Return response (and also set replybuffer with response).
uint8_t AndTech_MH900::getReply(const __FlashStringHelper *prefix, int32_t suffix1, int32_t suffix2, uint16_t timeout) {
  flushInput();
...
boolean AndTech_MH900::sendCheckReply(char *send, char *reply, uint16_t timeout) {
...
boolean AndTech_MH900::sendCheckReply(const __FlashStringHelper *send, const __FlashStringHelper *reply, uint16_t timeout) {
  getReply(send, timeout);
  return (strcmp_P(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, suffix, and newline.  Verify MH900 response matches reply parameter.
boolean AndTech_MH900::sendCheckReply(const __FlashStringHelper *prefix, char *suffix, const __FlashStringHelper *reply, uint16_t timeout) {
  getReply(prefix, suffix, timeout);
  return (strcmp_P(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, suffix, and newline.  Verify MH900 response matches reply parameter.
boolean AndTech_MH900::sendCheckReply(const __FlashStringHelper *prefix, int32_t suffix, const __FlashStringHelper *reply, uint16_t timeout) {
  getReply(prefix, suffix, timeout);
  return (strcmp_P(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, suffix, and newline.  Verify MH900 response matches reply parameter.
boolean AndTech_MH900::sendCheckReply(const __FlashStringHelper *prefix, int32_t suffix1, int32_t suffix2, const __FlashStringHelper *reply, uint16_t timeout) {
  getReply(prefix, suffix1, suffix2, timeout);
  return (strcmp_P(replybuffer, (prog_char*)reply) == 0);
}


boolean AndTech_MH900::parseReply(const __FlashStringHelper *toreply, 
...
boolean AndTech_MH900::sendParseReply(const __FlashStringHelper *tosend, 
